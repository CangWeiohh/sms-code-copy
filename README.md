...
监听 Mac 上收到的短信，命中关键词（默认：`验证码` / `动态密码` / `验证密码` / `校验码`）后自动提取短信验证码并复制到剪贴板，可选弹出系统通知。从此收到验证码后直接 ⌘V 粘贴即可。
...
./stop.sh     # 优雅停止，超过 15 秒未退出则强制结束
```

- `start.sh` 会检查 `run.pid` 防止重复启动；config.json 缺失时自动生成默认配置；
- 与 launchd 自启服务**互斥**：`start.sh` 检测到自启服务在运行会拒绝启动，避免两个进程同时轮询导致验证码重复复制；
- 启动失败时（如未授予完全磁盘访问权限），`start.sh` 会打印 `logs/console.log` 末尾的错误信息并以非零码退出；
- 前台调试可直接运行 `python3 sms_code_copy.py`（`Ctrl-C` 退出）。
...
| 配置项 | 默认值 | 说明 |
| --- | --- | --- |
| `keywords` | `["验证码", "动态密码", "验证密码", "校验码"]` | 关键词列表，命中任一即尝试提取验证码（不区分大小写） |
